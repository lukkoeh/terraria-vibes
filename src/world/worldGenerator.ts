  ...
  for (let lx = 0; lx < CHUNK_SIZE; lx++) {
    ...
    for (let y = y0; y < y1; y++) {
      ...
      if (y > h + soilDepth) {
        ...
        // air region above surface: do not overwrite existing trunks/wood/leaves
        if (existing !== Tile.Wood && existing !== Tile.Trunk && existing !== (Tile as any).Leaves) ch.set(lx, ly, Tile.Empty)
        ch.setBg(lx, ly, y > h - 6 ? Tile.Background : Tile.Empty)
      }
    }

    // Trees: only place once per column at the surface chunk to avoid re-placement
    // when vertically adjacent chunks get generated later while exploring.
    const surfaceChunkY = Math.floor(h / CHUNK_SIZE)
    if (chunkY === surfaceChunkY) {
